...
class TrackerConfig:
    ...
    def __init__(self):
        ...
        try:
            with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.txt'), 'r') as config:
                self.__read_config(config.readlines())
        except:
            # in case the config.txt file is missing, fall back to "real" default values
            pass
